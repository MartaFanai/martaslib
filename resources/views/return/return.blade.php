    ...
    <section class="content">
      ...
      <div class="card border-dark mb-3">
        ...
        <div class="card-header bg-danger">
          <h5>
              Return Section for <strong>{{ $name }}</strong> 
                <a href="{{ Route('return') }}" class="btn btn-warning ml-3"> Back </a> 
                @if($flag) 
                  <a href="{{ Route('return.all', $encodedIds) }}" class="btn btn-success float-right"> Return All Books </a>
                @endif
          </h5>
        </div>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    @elseif($remaining_days == 0)
                      <p class="text-blue">Last Day for Returning</p>
                      <?php $flag = 1; ?>
                    @else
                  ...
                  <td>
                    ...
                    @else
                      ...
                      <?php $duration = $rate->fees_duration ? 7 : 1; ?>

                      <a href="#" class="btn btn-danger" data-days="{{$remaining_days}}" data-rate="{{ $rate->fees }}" data-duration="{{ $duration }}" data-id="{{$r->id}}" data-toggle="modal" data-target="#modal-info" data-backdrop="static" data-keyboard="false">Pay Fine</a>
                    @endif
                  </td>
                </tr>
                @endforeach
          ...
          <div class="modal-content bg-info">
            ...
            <div class="modal-body">
              ...
              <div class="row">
                ...
                <div class="form-group col-sm-5"><h4>Amount To Pay</h4></div>
                <div class="form-group col-sm-3">
                  ...
                  <input type="number" min="0" id="fine" name="fine" class="form-control" style="size: 5px;"></div> 
...
$(document).ready(function(){
  ...
  $('#modal-info').on('show.bs.modal', function (e) {
    ...
    var amount = Math.round((rem_days / duration) * rate);

    document.getElementById('date').innerHTML = rem_days;
    document.getElementById('fine').value = amount;
    document.getElementById('fine').autofocus = true;
    document.getElementById('here').innerHTML = amount;

    var new_amount = amount; // Initialize new_amount variable with the initial value of amount
    var url = "{{ route('inv', ':id') }}";
        url = url.replace(':id', id);
        url += "?amount=" + new_amount;
        document.getElementById('print_receipt').href = url;

    document.getElementById('fine').addEventListener('change', function() {
        new_amount = document.getElementById('fine').value; // Update new_amount variable with the new value of fine input field
        var url = "{{ route('inv', ':id') }}";
        url = url.replace(':id', id);
        url += "?amount=" + new_amount;
        document.getElementById('print_receipt').href = url;
