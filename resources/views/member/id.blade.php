...
.address {
    ...
    font-size: 1.2em;
    position: absolute;
    bottom: 0; /* Align the bottom of the div with its containing element */
    line-height: 1.2em;
}
.last{
    width: 97%; 
    ...
    <section class="content">
    	<div class="container"  style="padding-left: 150px;">
    		<div class="main mt-4" id="printArea">
                <div id="image">
                    <p style="top: 15px; position: absolute; font-size: 27px; font-weight: bold; color: white; left: 4%; font-family: Arial, Helvetica, sans-serif;">Ramthar Veng Presbyterian Kohhran</p>
                <div class="top">
                    <p style="padding-top: 50px; font-size: 30px; font-weight: bold; color: white; font-family: Arial,sans-serif;">LIBRARY</p>
                    <div class="bg"></div>
                    <p style="top: 93px; position: absolute; font-size: 35px; font-weight: bold; color: white; left: 20%; font-family: Arial, Helvetica, sans-serif;">MEMBERSHIP ID</p>
                    
                </div>
                <div class="barcode" style="width: 70%;" >
                    <!-- Here @ is prefix to divert syntax error for wamp64 3.3.1 -->
                    {!! @DNS1D::getBarcodeSVG($member->id_number, "C39", 1.3, 25, '#2A3239') !!}
                </div>
                {{$member->image}}
                <img src="{{asset('storage/members/'.$member->image)}}" class="dp" >
                <div class="detail">
                    <div class="name" @if(strlen($member->name) <= 22 && strlen($member->name) >= 0) style="font-size: 1.7em;" @elseif(strlen($member->name) <= 25 && strlen($member->name) >= 23) style="font-size: 1.47em;" @elseif(strlen($member->name) <= 30 && strlen($member->name) >= 26) style="font-size: 1.25em;" @else style="font-size: 1.1em;" @endif >{{$member->name}}</div>               
                    <div class="fname">{{ empty($member->relationname) ? 'N/A' : $member->relation.' '. $member->relationname }}</div>               
                    {{-- <div class="sect">{{$member->section}} Section</div>                --}}
                    {{-- <div class="address">
                        {{$member->address}}, 
                        {!! empty($address->id_address_default) ? '' : '<br>' . $address->id_address_default !!}
                    </div> --}} 
